    ...
    return (
      <div className="flex flex-col h-full safe-top bg-deck-bg overflow-hidden">
        ...
        <header className="flex items-center gap-2 px-3 py-2 bg-deck-surface border-b border-deck-border shrink-0">
          <button onClick={goUp} className="p-1.5 -ml-1 rounded active:bg-deck-border/30">
            <IconBack size={16} />
          </button>
          ...
          <div className="flex-1 min-w-0 min-h-0 overflow-hidden">
            {usesNative(agent) ? (
              agent.preset === 'auto'
                ...
                : <NativeChat key={`${agentId}:${nativeDriver(agent)}`} agentId={agentId} cwd={agent.workingDir} driver={nativeDriver(agent)} autoRouted={!!agent.autoProfile} onAgentChange={setAgent} />
            ) : (
              <TerminalView
                key={agentId}
                ref={terminalApiRef}
