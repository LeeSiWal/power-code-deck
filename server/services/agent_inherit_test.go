...
func TestInheritedNativeConfig(t *testing.T) {
	...
	if m, md := s.inheritedNativeConfig("/proj/a"); m != "" || md != "" {
	...
	if m, md := s.inheritedNativeConfig("/proj/a"); m != "claude-opus-4-8" || md != "bypassPermissions" {
	...
	if m, md := s.inheritedNativeConfig("/proj/a"); m != "claude-sonnet-5" || md != "plan" {
	...
	if m, md := s.inheritedNativeConfig("/proj/a"); m != "claude-fable-5" || md != "" {
