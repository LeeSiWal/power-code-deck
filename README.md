...
- **알림** — 작업 완료·승인 요청·정체·에러가 발생하면 화면 안 토스트와 하단 탭 배지로 알리고, 활성 기기에는 웹 푸시를 보냅니다. 알림을 누르면 해당 세션으로 이동합니다. 서버는 읽음/안읽음을 SQLite에 저장합니다.
  > 지난 알림을 모아보는 **알림 센터 화면은 아직 없습니다** — 배지와 토스트만 있습니다. 로드맵 항목입니다.
...
- 타일에 Git 브랜치·리스닝 포트 표시, 헤더에 프로젝트 추가
...
상세 로드맵은 [ROADMAP.md](ROADMAP.md) 참고. 큰 줄기는:
- 안정화 — 전체 허용 모드 불안정, 세션 재진입 시 활동 표시 누락
- 알림 센터 — 지난 알림 목록·삭제, 알림에서 해당 작업으로 바로 이동
- 승인 정책 — "앞으로도 허용"(허용 목록)
- 관제실 활동 표시 · 할일 체크리스트
