...
- **웹 터미널** — xterm.js 기반 실시간 단일 Interactive Terminal (한글 직접 입력), 모바일·대화형 메뉴용 방향키 툴바, tmux 세션으로 프로세스 유지
...
### Stable in v0.2
- Interactive Terminal (단일 터미널, 한글 직접 입력)
- 방향키/제어키 툴바 (데스크톱·모바일)
...
PowerCodeDeck은 하나의 **Interactive Terminal**을 사용합니다. 별도의 Chat/Raw 모드나 프롬프트 입력창은 없습니다.

- 모든 입력은 실제 터미널에 직접 전달됩니다. **한글도 터미널에 바로 입력**할 수 있습니다.
- Claude의 선택지, 승인 요청(y/n), 방향키, Tab, Esc, Ctrl+C는 터미널에서 그대로 조작합니다.
- 하단 **방향키/제어키 툴바**로 `↑ ↓ ← → Enter Esc Tab ⇧Tab y n Ctrl+C Ctrl+D`를 전송할 수 있습니다. 물리 키보드가 없는 모바일에서 대화형 메뉴를 조작할 때 특히 유용합니다.
- 모바일에서는 툴바의 **⌨ 버튼**으로 터미널에 포커스를 주면 키보드가 올라와 직접 입력할 수 있습니다.
- 터미널에 붙여넣기(paste)하면 여러 줄/긴 텍스트도 그대로 전달됩니다.

> 모든 입력·선택지·방향키·한글을 하나의 터미널에서 처리하고, 방향키 툴바가 물리 키보드를 보조합니다 — 라는 단순한 규칙입니다.
...
| C→S | `terminal:pasteSubmit` | 텍스트를 붙여넣고 Enter 전송 (서버가 bracketed paste로 래핑) — 레거시 WS API, 현재 UI는 미사용 |
| C→S | `terminal:pasteOnly` | 텍스트를 붙여넣기만 (Enter 없음) — 레거시 WS API, 현재 UI는 미사용 |
