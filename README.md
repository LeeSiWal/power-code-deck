...
3. WSL 설치가 처음이라 **재부팅이 필요하면**, 스크립트가 `Reboot now? [Y/n]` 라고 물어봅니다 → **`Y` (Enter)**.
...
> - **CPU 가상화(VT-x/AMD-V)가 꺼져 있어 WSL2가 안 뜨는 PC**는, 스크립트가 자동으로 **WSL1**(가상화 불필요)로 전환해 설치를 계속합니다. 그래도 안 되면 BIOS/UEFI에서 가상화를 켜고 재부팅한 뒤 다시 실행하세요.
