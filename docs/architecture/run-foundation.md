...
`worker.go` executes an explicit start using the Antigravity adapter. The
...
The authenticated web UI exposes `/runs`: create and start an Antigravity Run,
...
error before claiming an attempt. Multiple tasks and dependencies, scheduling
