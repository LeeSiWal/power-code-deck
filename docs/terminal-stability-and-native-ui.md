...
> **함의**: 우리는 그들이 폐기한 설계를 쓰고 있다.
>
> **정정 (2026-07-17)**: 이 문서 초안은 "우리는 이미 서버에서 ghostty core를 돌리므로 도약이 싸다"고 적었으나 **사실이 아니다**. 확인 결과 서버(Go)에는 이뮬레이터가 없다 — `go.mod`에 VT 라이브러리가 없고, ghostty는 클라이언트 쪽 흔적이며 그마저 자체 DOM 렌더러로 대체됐다. 따라서 이 도약에는 **서버용 VT 이뮬레이터를 새로 들여야 한다** (결정 #2). 비용은 초안이 말한 것보다 크다.
...
**후보** (서버에 이뮬레이터가 없으므로 새로 들여야 한다):

| 후보 | 장점 | 단점 |
|---|---|---|
| **node 사이드카 + `@xterm/headless`** | 클라이언트와 **완전히 같은 파서·같은 폭 테이블**. VS Code가 하는 그대로. 직렬화기/렌더러 불일치가 원천적으로 없음 | Go 단일 바이너리에 node 프로세스가 붙는다 — 설치·배포·수명주기 관리 비용. `pcd` 단일 바이너리 철학과 충돌 |
| **Go VT 라이브러리** | 단일 바이너리 유지 | **폭 테이블·리플로우 semantics가 xterm과 다르다** → 이 문서가 경고한 바로 그 불일치를 서버-클라 사이에 새로 만든다. 직렬화 충실도를 직접 검증해야 함 |
| **직접 구현** | 필요한 것만 | 터미널 이뮬레이터를 직접 쓰는 일이다. 비추 |

이 선택이 결정 #2다. 폭 일치가 이 설계의 핵심 전제이므로 **node 사이드카가 기술적으로는 정공법**이지만, 단일 바이너리 배포를 포기할 만한 가치가 있는지는 제품 판단이다.
