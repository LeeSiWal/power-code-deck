...
│                   Go 서버 (단일 바이너리)                   │
│  ┌────────┐ ┌────────┐ ┌─────────────┐ ┌──────────────┐ │
│  │ Router │→│Handler │→│SessionEngine│→│ 내부 PTY 엔진 │ │
│  └────────┘ └────────┘ └─────────────┘ └──────────────┘ │
│       ↕            ↕         ↕                          │
│  ┌────────┐  ┌─────────┐ ┌───────┐                     │
│  │  Auth  │  │ WS Hub  │ │SQLite │                     │
...
> **세션 엔진(SessionEngine):** 핸들러·WS Hub·에이전트 서비스는 PTY를 직접
> 호출하지 않고 `SessionEngine` 인터페이스만 사용합니다. 현재 구현은 pcd 서버가
> 각 세션의 shell/에이전트 프로세스와 PTY를 직접 소유하는 내부 PTY 엔진
> `InternalPtySessionEngine`이며(tmux 불필요), 이후 별도 `pcd-sessiond` 데몬 등으로
> 교체 가능합니다. 핵심 불변식은 **"Detach ≠ Kill"** — 브라우저가 나가도 viewer만
> 분리되고 에이전트 프로세스는 죽지 않습니다. 최근 출력은 스크롤백 링버퍼에 저장되어
> 재접속 시 재생됩니다. 자세한 내용: [docs/session-engine.md](docs/session-engine.md).
...
```sql
agents (id, preset, name, tmux_session (legacy, 미사용), working_dir, command, args,
...
```
[PTY 세션 (pcd 서버 소유)] → WS Hub → terminal:output → [xterm.js]
     ↑                                              ↓
...
세션은 pcd 서버가 직접 소유하는 내부 PTY 엔진(`InternalPtySessionEngine`)에서 실행됩니다 (tmux 불필요).

```
- 서버가 각 세션의 shell/에이전트 프로세스와 PTY를 직접 소유
- 브라우저 연결 해제 시 viewer만 분리, 프로세스는 계속 실행 (Detach ≠ Kill)
- Kill / Restart / Delete 만 프로세스를 종료
- 최근 출력은 세션별 스크롤백 링버퍼에 저장 → 재접속 시 재생
  (크기: POWERCODEDECK_SESSION_SCROLLBACK_BYTES, 기본 524288)
```
...
- **내부 PTY 세션 엔진**: pcd 서버가 에이전트별 PTY 프로세스를 직접 소유, 웹에서 실시간 I/O, Detach ≠ Kill (브라우저를 닫아도 프로세스 유지), 스크롤백 링버퍼로 재접속 재생
